...
Status SparseCooFomatRep::Copy(const DataTransferManager& data_transfer_manager, const AllocatorPtr& allocator,
  ...
  auto rep_copy = std::make_unique<SparseCooFomatRep>(indices_.Shape(), allocator);
...
Status SparseCooBuilder::GetOrCreate(bool linearized, SparseCooFomatRep*& result) {
  ORT_RETURN_IF_NOT(allocator_ != nullptr, "Must have an allocator set with Sparse Tensor instance");

  if (rep_->get()) {
    result =  static_cast<SparseCooFomatRep*>(rep_->get());
    return Status::OK();
  }

  const auto nnz = sp_->Values().Shape().Size();
  if (linearized) {
    result = new SparseCooFomatRep({nnz}, allocator_);
    rep_->reset(result);
  } else {
    result = new SparseCooFomatRep({nnz, 2}, allocator_);
    rep_->reset(result);
  }
  return Status::OK();
}

Status SparseCooBuilder::GetOrCreate(const TensorShape& indices_shape, void* indices_data, SparseCooFomatRep*& result) {
  ...
  result = new SparseCooFomatRep(indices_shape, sp_->Location(), indices_data);
  rep_->reset(result);
  return Status::OK();
}
